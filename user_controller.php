<?php 
    ...
    if(isset($_POST['Register'])){
        ...
        $user_exist = "SELECT * FROM user_register WHERE email = '$b' OR mobile  = '$c'";
        ...
        if($result){
            ...
            if($result -> num_rows > 0){
                ...
                if($data['email'] == $b){
                    ...
                    ";
                } elseif($data['mobile'] == $c){
                    echo "
                        <script>
                            alert(' $data[mobile] - This Mobile No. Already Exits.... ');
                            window.location.href='index.php';
                        </script>
                    ";
                }
                ...
                $ins = "INSERT INTO user_register SET full_name = '$a', email = '$b', mobile = '$c', image = '$d_nm', password = '$password'";
                if($db -> query($ins)){
                    echo "
                        <script>
    ...
    if(isset($_POST['Login'])){
        ...
        $user_exist = "SELECT * FROM user_register WHERE email = '$a' OR mobile  = '$b'";
        $result = $db -> query($user_exist);
        ...
        if($result){
            ...
            if($result -> num_rows == 1){
                ...
                if(password_verify($b, $data['password'])){
                ...
                echo "
                    <script>
                        alert('Email Or Mobile No. is incorrect, Please fillup again.....');
                        window.location.href='index.php';
                    </script>
                ";
    ...
    if(isset($_POST['upd_profile'])){
        ...
        if(isset($_FILES['img']['name']) && $_FILES['img']['name']!=""){
            ...
            $upd = "UPDATE user_register SET full_name = '$a', email = '$b', mobile = '$c', image = '$d_nm' WHERE user_id = '$req'";
            if($db -> query($upd)){
                echo "
                    <script>
                        alert('Update Done....');
            ...
            $upd = "UPDATE user_register SET full_name = '$a', email = '$b', mobile = '$c' WHERE user_id = '$req'";
            if($db -> query($upd)){
                echo "
                    <script>
                        alert('update Done....');
    ...
    if(isset($_POST['order'])){
        ...
        $f_p = implode(", ", $p_nm);
        $ins = "INSERT INTO orders SET uid = '$a', u_name = '$b', mobile = '$c', landmark = '$d' , town = '$e' , address = '$f', t_product = '$f_p', t_price = '$t_p'";
        if($db -> query($ins)){
